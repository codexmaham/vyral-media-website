...
void main(){vec4 color;mainImage(color,gl_FragCoord.xy);gl_FragColor=color;}`;
      ...
      const loop = (t: number) => {
        ...
        try {
          (renderer as { render: (opts: { scene: unknown }) => void }).render({ scene: mesh });
      ...
      cleanupFunctionRef.current = () => {
        ...
        try {
          const canvas = (renderer as { gl: { canvas: HTMLCanvasElement; getExtension: (n: string) => { loseContext: () => void } | null } }).gl.canvas;
          const glCtx = (renderer as { gl: { canvas: HTMLCanvasElement; getExtension: (n: string) => { loseContext: () => void } | null } }).gl;
